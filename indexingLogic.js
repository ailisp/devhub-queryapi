...
async function createDump(
  ...
  try {
    ...
    await context.graphql(
      `
        mutation CreateDump($dump: bo_near_devhub_v36_dumps_insert_input!) {
          insert_bo_near_devhub_v36_dumps_one(
            object: $dump
          ) {
            receipt_id
          }
...
async function createPost(context, { id, parent_id, author_id }) {
  ...
  try {
    ...
    await context.graphql(
      `
      mutation CreatePost($post: bo_near_devhub_v36_posts_insert_input!) {
        insert_bo_near_devhub_v36_posts_one(object: $post) {id}
      }
      `,
      mutationData
    );
...
async function createPostSnapshot(
  ...
  try {
    ...
    await context.graphql(
      `
      mutation CreatePostSnapshot($post_snapshot: bo_near_devhub_v36_post_snapshots_insert_input!) {
        insert_bo_near_devhub_v36_post_snapshots_one(object: $post_snapshot) {post_id, block_height}
      }
      `,
      mutationData
    );
  ...
  try {
    ...
    await context.graphql(
      ...
      mutation CreateLike($author_id: String, $post_id: Int, $ts: numeric) {
        insert_bo_near_devhub_v36_likes_one(
